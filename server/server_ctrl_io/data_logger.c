...
#include <string.h>
#include <time.h>
#include <inttypes.h> /* Este include j· estava aqui, o que È Ûtimo! */
...
int DataLogger_init(const char* db_path)
{
    if (logger.initialized) {
        printf("DataLogger: J√° inicializado\n");
        return 0;
    }

    int rc = sqlite3_open(db_path, &logger.db);
    if (rc != SQLITE_OK) {
        printf("DataLogger: N√£o foi poss√≠vel abrir o banco de dados %s: %s\n", db_path, sqlite3_errmsg(logger.db));
        return -1;
    }
                                ...
                                AnalogRecord** records, int* count)
{
    ...
    /* Alocar mem√≥ria para os registros */
...
void DataLogger_printAnalogStats(const char* channel, uint64_t hours_back)
{
    ...
    if (DataLogger_queryAnalogData(channel, start_time, now, &records, &count) == 0) {
        if (count > 0) {
            ...
            /* ---------- CORRE«√O 1 ---------- */
            printf("Status para %s (ultimas %" PRIu64 " horas):\n", channel, hours_back);
            printf("  Records: %d\n", count);
            printf("  Min: %.2f\n", min_val);
            printf("  Max: %.2f\n", max_val);
            printf("  Avg: %.2f\n", sum / count);
            ...
            /* ---------- CORRE«√O 2 ---------- */
            printf("N√£o foram encontrados dados para %s nas ultimas %" PRIu64 " horas\n", channel, hours_back);
        }
...
void DataLogger_printDigitalStats(const char* channel, uint64_t hours_back)
{
    ...
    if (DataLogger_queryDigitalData(channel, start_time, now, &records, &count) == 0) {
        if (count > 0) {
            ...
            /* ---------- CORRE«√O 3 ---------- */
            printf("Stats for %s (last %" PRIu64 " hours):\n", channel, hours_back);
            printf("  Records: %d\n", count);
...
void DataLogger_cleanupOldData(uint64_t days_to_keep)
{
    ...
    /* ---------- CORRE«√O 5 ---------- */
    snprintf(sql, sizeof(sql), "DELETE FROM analog_data WHERE timestamp < %" PRIu64, cutoff_time);
    sqlite3_exec(logger.db, sql, NULL, NULL, NULL);

    /* ---------- CORRE«√O 6 ---------- */
    snprintf(sql, sizeof(sql), "DELETE FROM digital_data WHERE timestamp < %" PRIu64, cutoff_time);
    sqlite3_exec(logger.db, sql, NULL, NULL, NULL);

    /* ---------- CORRE«√O 7 ---------- */
    printf("DataLogger: Cleaned up data older than %" PRIu64 " days\n", days_to_keep);
}
